...
public class AppLaunch extends Window {
	...
	public AppLaunch() {
		...
		if (Config.LINK_ACUITY) {
			...
			String timestamp = DateTimeFormatter.ofPattern("MMM dd yyyy hh:mma").format(LocalDateTime.now()).replace(".", "");
			try {
				new Mailer().sendMail("CLASSROOM MANAGER SESSION START",
						Config.CENTRE_NAME + " re-authenticated on " + timestamp);
			} catch (BindException be) {
				JOptionPane.showMessageDialog(null, "Application already running.  First instance must be authenticated.", "Error", JOptionPane.ERROR_MESSAGE);
				System.exit(0);
			} catch (Exception e) {
				e.printStackTrace();
			}			
...
//                	.addComponent(helpButton, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    ...
                    .addComponent(configurationsButton, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addComponent(editStudentsButton, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    .addComponent(startClassButton, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
//                    .addComponent(logButton, GroupLayout.DEFAULT_SIZE, GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                    )
