...
/// RIGHT PANE ///
	/** There are additional methods outside of overridden methods used throughout this window.  This is initialized 
	 * as CheckedInStudentsTableModel rather than DefaultTableModel to avoid the need for casting. */
					...
					if (Config.LINK_ACUITY) {
						try {
							new Mailer().sendAttachment(
									"CLASS LOG FOR " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("MMM dd, yyyy")).toUpperCase(),
									"",
	...
	protected void initComponents() {
		...
		checkedInStudentsTable = new ProgramTable(checkedInStudentsTableModel) {
			...
			public Component prepareRenderer(TableCellRenderer renderer, int row, int column) {
				...
				int statusIdCol = 10;
				int colorCol = 12;
				String studentNotes = (String) this.getModel().getValueAt(convertRowIndexToModel(row), notesCol); // toString() requires object to not be null
				Color indicatorColor = Color.decode((String) this.getModel().getValueAt(convertRowIndexToModel(row), colorCol));
				...
				if (!isCellSelected(row, column)) {
					if (column == notesCol && studentNotes != null && studentNotes.length() > 0) {
						...
						int statusID = (int) this.getModel().getValueAt(convertRowIndexToModel(row), statusIdCol); // status id
						switch (statusID) {
		...
		TableRowSorter<TableModel> checkedInTableSorter = new TableRowSorter<TableModel>(checkedInStudentsTableModel) {
			@Override
			public boolean isSortable(int column) { return (column == COLS - 1); }
		};
		...
		checkinButton.addMouseListener(new MouseAdapter() {
			@Override
			public void mousePressed(MouseEvent me) { checkinBtnMouseClicked(me); }
		});	
		 ...
		 *A user normally clicks the mouse button when selecting or double clicking an icon
		 */
		...
		checkedInStudentsTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent me) {
				if (SwingUtilities.isRightMouseButton(me)) {
					...
					checkOutPopupWithText.addMouseListener(new MouseAdapter( ) {
						...
						public void mousePressed(MouseEvent me) {
							if (!SwingUtilities.isRightMouseButton(me) && checkedInStudentsTable.getSelectedRow() >= 0) {
				...
				} else if (me.getClickCount() == 2 && me.getButton() == MouseEvent.BUTTON1) { // ONLY ALLOW DOUBLE CLICK IF STUDENT IS IN WARNING 
					int rowIndex = checkedInStudentsTable.convertRowIndexToModel(checkedInStudentsTable.getSelectedRow());
					if (rowIndex >= 0 && (int) checkedInStudentsTableModel.getValueAt(rowIndex, 8) <= Config.WARNING_TIME)
						checkOut();						
					else
						messages.append("Right click to check out when time \nleft is more than " + Config.WARNING_TIME + "\n");
				}
			}
	...
	private void checkIn() {
		...
		for (int i : effectiveRows) {
			...
			if (!checkedInStudentsMap.keySet().contains(keyToCheck)) {
				checkedInStudentsMap.put(keyToCheck, new Student(allStudentsMap.get(keyToCheck)));	// creates new Student in checked-in students pane with time information
				if (Config.LINK_ACUITY)
...
//					messages.append(allStudentsMap.get(keyToCheck).getFName() + "'s birthday is on " + allStudentsMap.get(keyToCheck).getBirthday().substring(0, 5) +".\n");
				
				System.out.println(allStudentsMap.get(keyToCheck).getName() + " " + allStudentsMap.get(keyToCheck).daysUntilBirthday());
	...
	class SendMessage implements Runnable {
		...
		public void run() {
			try {
				TextConverter textProcessor = new TextConverter(allStudentsMap.get(keyToCheck).getFName(), allStudentsMap.get(keyToCheck).getLName());
	...
	class AppointmentAlert implements Runnable {
		...
		public AppointmentAlert(Log log) { this.log = log; }
		...
		public void run() {
			try {
				...
				// GENERATE EMAIL
				String date = log.getSignIn().getMonth() + " " + log.getSignIn().getDayOfMonth() + ", " + log.getSignIn().getYear();
	...
	class CheckedInStudentsTableModel extends DefaultTableModel {
		...
		@Override
		public int getRowCount() { return studentMap == null ? 0 : studentMap.size(); }
