...
public class Checkin extends Window {
	...
	private String account_sid = env.get("KUMON_TWILIO_ACCOUNT_SID");
	private String auth_token = env.get("KUMON_TWILIO_AUTH_TOKEN");
	private String from = env.get("KUMON_TWILIO_FROM");
	...
	protected void initComponents() {
		...
		checkedInStudentsTable.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent me) {
				...
				} else if (me.getClickCount() == 2 && me.getButton() == MouseEvent.BUTTON1) { // ONLY ALLOW DOUBLE CLICK IF
																																											// STUDENT IS IN WARNING
	...
	private void checkIn() {
		...
		for (int i : effectiveRows) {
			...
			if (!checkedInStudentsMap.keySet().contains(keyToCheck)) {
				...
				System.out.println(
						allStudentsMap.get(keyToCheck).getName() + " " + allStudentsMap.get(keyToCheck).daysUntilBirthday());
