...
The /remote_db/ branch moves the database from localhost to the cloud. Since the database is no longer local, session variables (namely, the connection timeout) cannot be changed. Therefore, a singleton connection cannot be used, and all transactions require a new instance of dao.DBConnection.

For security, the credentials have been moved to environment variables. The following environment variables need to be set:
